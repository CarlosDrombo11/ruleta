...
public static loadConfig(): AppConfig {
  try {
    ...
    if (!saved) return JSON.parse(JSON.stringify(this.DEFAULT_CONFIG));

    const parsed = JSON.parse(saved) as AppConfig;

    // HAZ MERGE SIEMPRE (aunque cambie la versión)
    const merged = this.mergeConfig(this.DEFAULT_CONFIG, parsed);

    if (version !== this.CURRENT_VERSION) {
      // sube versión y persiste el merge
      localStorage.setItem(this.VERSION_KEY, this.CURRENT_VERSION);
  ...
  private static mergeConfig(defaultConfig: AppConfig, savedConfig: any): AppConfig {
    // Crear copia profunda del config por defecto
    const merged: AppConfig = JSON.parse(JSON.stringify(defaultConfig));
    ...
    Object.keys(savedConfig).forEach(key => {
      ...
      if (defaultConfig[configKey] !== undefined) {
        if (typeof defaultConfig[configKey] === 'object' &&
            ...
            !Array.isArray(defaultConfig[configKey])) {
          // Merge objetos anidados
          merged[configKey] = {
            ...(defaultConfig[configKey] as any),
            ...savedConfig[key]
          } as any;
        } else {
          // Asignar valores primitivos y arrays
          merged[configKey] = savedConfig[key];
        }
      }
    });
  ...
  public static getDefaultConfig(): AppConfig {
    // Usar JSON.parse(JSON.stringify()) para crear una copia profunda
    return JSON.parse(JSON.stringify(this.DEFAULT_CONFIG));
  }
}
