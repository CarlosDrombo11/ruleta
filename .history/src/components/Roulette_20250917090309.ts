...
export class Roulette extends EventEmitter {
  // Core systems - Usar definite assignment assertion (!)
  ...
  // DOM elements - Usar definite assignment assertion (!)
  ...
  private initialized = false; // Cambié el nombre para evitar conflicto
  ...
  constructor(container: HTMLElement) {
    ...
    this.initialized = true; // Usar el nuevo nombre
  }
  ...
  private initializeDOM(): void {
    ...
    // Get DOM references - Verificar que existan
  ...
  private initializeSystems(): void {
    // Initialize core systems - Ahora que canvas y participantsSection están definidos
  ...
  public async loadExcelFile(file: File): Promise<void> {
    try {
      this.updateStatusIndicator('spinning' as RouletteState); // Using valid state
  ...
  private setupConfigEventListeners(): void {
    ...
    const oldListeners = this.configPanel.querySelectorAll('[data-listener]');
    ...
    if (dropArea) {
      dropArea.setAttribute('data-listener', 'true');
    ...
    // Title controls con verificación
    const titleText = this.configPanel.querySelector('#titleText') as HTMLInputElement;
    if (titleText) {
      titleText.setAttribute('data-listener', 'true');
      titleText.addEventListener('input', (e) => {
        e.stopPropagation();
        const value = (e.target as HTMLInputElement).value;
        this.config.title.text = value;
        this.applyTitleChanges();
        console.log('Título cambiado:', value);
      });
      titleText.addEventListener('change', (e) => {
        e.stopPropagation();
        const value = (e.target as HTMLInputElement).value;
        this.config.title.text = value;
        this.applyTitleChanges();
    ...
    if (titleSize && titleSizeValue) {
      titleSize.setAttribute('data-listener', 'true');
    ...
    backgroundRadios.forEach((radio, index) => {
      ...
      radio.addEventListener('change', (e) => {
        ...
        if (target.checked) {
          ...
          this.applyConfiguration();
          console.log('Tipo de fondo cambiado:', target.value);
          
          // Actualizar clases visuales
          const radioItems = this.configPanel.querySelectorAll('.config-radio-item[data-bg-type]');
    ...
    animationRadios.forEach(radio => {
      ...
      radio.addEventListener('change', (e) => {
        ...
        if (target.checked) {
          ...
          console.log('Tipo de animación cambiado:', target.value);
          
          // Actualizar clases visuales
          const radioItems = this.configPanel.querySelectorAll('.config-radio-item[data-anim-type]');
          radioItems.forEach(item => item.classList.remove('selected'));
          target.closest('.config-radio-item')?.classList.add('selected');
        }
      });
    ...
    audioRadios.forEach(radio => {
      ...
      radio.addEventListener('change', (e) => {
        ...
        if (target.checked) {
          ...
          console.log('Tipo de audio cambiado:', target.value);
          
          // Actualizar clases visuales
          const radioItems = this.configPanel.querySelectorAll('.config-radio-item[data-audio-type]');
          radioItems.forEach(item => item.classList.remove('selected'));
          target.closest('.config-radio-item')?.classList.add('selected');
        }
      });
    ...
    modeRadios.forEach(radio => {
      ...
      radio.addEventListener('change', (e) => {
        ...
        const target = e.target as HTMLInputElement;
        if (target.checked) {
          this.config.mode = target.value as any;
          ...
          console.log('Modo cambiado:', target.value);
          
          // Actualizar clases visuales
          const radioItems = this.configPanel.querySelectorAll('.config-radio-item[data-mode]');
          radioItems.forEach(item => item.classList.remove('selected'));
          target.closest('.config-radio-item')?.classList.add('selected');
        }
      });
    ...
    // Rangos con mejor manejo
    this.setupRangeControl('#animationIntensity', '#intensityValue', (value: number) => {
      this.config.animations.intensity = value;
      this.particleSystem.setIntensity(value);
      console.log('Intensidad de animación:', value);
    });

    this.setupRangeControl('#animationSpeed', '#speedValue', (value: number) => {
      this.config.animations.speed = value;
      this.particleSystem.setSpeed(value);
      console.log('Velocidad de animación:', value);
    });

    this.setupRangeControl('#audioVolume', '#volumeValue', (value: number) => {
      this.config.audio.volume = value;
      this.audioEngine.setVolume(value);
      console.log('Volumen de audio:', Math.round(value * 10));
    }, (value: number) => Math.round(value * 10).toString());

    this.setupRangeControl('#spinDuration', '#durationValue', (value: number) => {
      this.config.timing.spinDuration = value;
      console.log('Duración del sorteo:', value);
    }, (value: number) => `${value}s`);

    this.setupRangeControl('#autoInterval', '#intervalValue', (value: number) => {
      this.config.timing.autoInterval = value;
      console.log('Intervalo automático:', value);
    }, (value: number) => `${value}s`);

    // Otros controles
    this.setupOtherControls();

    console.log('✅ Event listeners del panel de configuración configurados');
  }
  ...
  private setupRangeControl(
    ...
    const range = this.configPanel.querySelector(rangeSelector) as HTMLInputElement;
    const valueDisplay = this.configPanel.querySelector(valueSelector) as HTMLElement;
    
    if (range && valueDisplay) {
      range.setAttribute('data-listener', 'true');
      
      const updateValue = (e: Event) => {
        e.stopPropagation();
        const value = parseFloat((e.target as HTMLInputElement).value);
        onChange(value);
        valueDisplay.textContent = formatValue ? formatValue(value) : value.toString();
      };
  ...
  private setupOtherControls(): void {
    ...
    if (startAutoBtn) {
      startAutoBtn.setAttribute('data-listener', 'true');
    ...
    if (saveConfigBtn) {
      saveConfigBtn.setAttribute('data-listener', 'true');
  ...
  public isInitialized(): boolean {
    return this.initialized; // Usar el nuevo nombre de la propiedad
  }
}