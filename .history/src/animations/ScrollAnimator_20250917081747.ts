...
export class ScrollAnimator {
  ...
  private createScrollAnimation(targetPosition: number, duration: number): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      // Usar Promise.resolve() para manejar correctamente el chaining
      const phase1Promise = new Promise<void>((phase1Resolve, phase1Reject) => {
        this.currentAnimation = animate({
          from: 0,
          to: phase1Distance,
          duration: phase1Duration,
          ease: [0.25, 0.1, 0.25, 1], // Bezier curve para easeOut
      ...
      phase1Promise
        .then(() => {
          ...
          return new Promise<void>((phase2Resolve, phase2Reject) => {
            this.currentAnimation = animate({
              from: phase1Distance,
              to: targetPosition,
              duration: phase2Duration,
              ease: [0.55, 0.055, 0.675, 0.19], // Bezier curve para easeOutCubic
              onUpdate: (value: number) => {
                this.setScrollPosition(value);
              },
              onComplete: () => {
