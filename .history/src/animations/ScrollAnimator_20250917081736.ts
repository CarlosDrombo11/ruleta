...
export class ScrollAnimator {
  ...
  private renderParticipants(): void {
    ...
    // Duplicar lista para efecto de loop infinito
    const duplicatedList = [...this.participants, ...this.participants, ...this.participants];
  ...
  public async syncWithRoulette(duration: number, winner: Participant): Promise<void> {
    ...
    try {
      ...
      const initialSpeed = this.calculateInitialSpeed(targetPosition, duration);
      ...
      await this.createScrollAnimation(targetPosition, duration, initialSpeed);
  ...
  private calculateInitialSpeed(targetPosition: number, duration: number): number {
    // Calcular velocidad inicial basada en la distancia y duración
    // Considerando desaceleración exponencial
    return targetPosition / (duration * 0.4); // Factor de ajuste para la curva
  }

  private createScrollAnimation(targetPosition: number, duration: number, initialSpeed: number): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      phase1.then(() => {
        ...
        const remainingDistance = targetPosition - phase1Distance;
